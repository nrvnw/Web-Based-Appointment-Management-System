...
<head>
    ...
    <link rel="stylesheet" href="o_filechecking.css">
...
<?php
    ...
    if(isset($_GET['id'])) {
      ...
      if(mysqli_num_rows($result) > 0) {
        ...
        echo "<p class=\"lbl_flchk\">File Checking</p>";
        ...
        echo '<form method="get" action="preview_pdf.php">
        <input type="hidden" name="id" value="' . $id . '">
        <button class="PrvFile" onclick="window.open(\'preview_pdf.php?id=' . $id . '\', \'_blank\'); return false;">Preview PDF</button>
        </form>';
  


        echo "<form method='post' action='uploaddoc.php' enctype='multipart/form-data'>";
        echo "<input type='hidden' name='appointment_id' value='" . $row['id'] . "'>";
        echo "<label for='file-upload' class='adfl'>Change File</label>";
        echo "<input type='file' id='file-upload' name='file' style='display:none;' onchange=\"document.getElementById('filename').value=this.value\">";
        echo "<a id=\"preview-link\" class=\"Newfl\"></a>";
    ...
    echo"</div>";
        ...
        echo"<a href=\"FileChecking.php\" class=\"bck\">Back</a>";
        echo"<a href=\"uploadtocompleted.php\" class=\"cnf\">Confirm</a>";
        echo "<form method='post'  action='uploadtocompleted.php' enctype='multipart/form-data'>";
        echo "<input type='hidden' name='appointment_id' value='" . $row['id'] . "'>";
        echo "<button type='submit' class=\"cnf\">Confirm</button>";
        echo "</form>";
