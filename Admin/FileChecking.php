...
<head>
    ...
    <link rel="stylesheet" href="FileChecking.css">
...
<body>
    <div class="container">
        ...
        <p class="lbl_flchk">File Checking</p>
  ...
  $sql = "SELECT * FROM filecheck WHERE receipt_number = '$receipt_number'";
  ...
  if(mysqli_num_rows($result) > 0) {
      ...
      while($row = mysqli_fetch_assoc($result)) {
          echo "<tr><td>" . $row['receipt_number'] . "</td><td>" . $row['date'] . "</td><td>" . $row['time'] . "</td><td>" . $row['requirements'] . "</td><td>" . $row['name'] . "</td><td>" . $row['email'] . "</td><td>" . $row['phone'] . "</td><td><a href='o_filechecking.php?id=" . $row['id'] . "' class=\"act\">Open</a></td></tr>";
      }
      echo "</table>";
  }
  else {
    ...
    // select all the data from the filecheck table
    $sql = "SELECT * FROM filecheck";
    $result = mysqli_query($conn, $sql);
